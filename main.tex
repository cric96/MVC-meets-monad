...
\begin{frame}{Recap}
\begin{itemize}
\item We clearly isolate side effects in monads (Task, Observable)
\item Then we can arbitrarily compose them to build an imperative like behaviour
\item Model are side-effect free
\item The program built is referential transparent
\item Concurrency is handled with monads themself (via shift, Fiber, ...)
\end{itemize}
